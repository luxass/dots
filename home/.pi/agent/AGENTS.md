...
- Use the `dot` CLI for setup, package checks, symlink checks, and diagnostics
  when relevant.
- Do not run `dot doctor` automatically. Run it when the user asks for
  ...
  management, Stow behavior, or runtime setup.
...
- Managed Pi files live under `home/.pi/` and are intended to be stowed into
  `$HOME/.pi`. This is a lean setup: settings, keybindings, and these notes
  only, no custom extensions.
- Prefer editing tracked configuration. Shared Agent Skills live under
  `home/.agents/skills/` and are managed with `dot skills`, not duplicated here.
